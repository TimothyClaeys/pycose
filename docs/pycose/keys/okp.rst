...
COSE OKP keys can be used for signing and verifying :class:`pycose.messages.sign1message.Sign1Message` and
...
COSE OKP keys can be created using the :class:`~pycose.keys.okp.OKPKey` class or from a standard Python
...
Alternatively you can use the :meth:`~pycose.keys.okp.OKPKey.generate_key` method. It generates a random
COSE OKP Key for a given curve. Valid curves are :class:`~pycose.keys.curves.X22519`, :class:`~pycose.keys.curves.X448`,
:class:`~pycose.keys.curves.ED25519`, and :class:`~pycose.keys.curves.ED448`.
...
:class:`~pycose.keys.keyparam.KpKty`, :class:`~pycose.keys.keyparam.OKPKpCurve`, and either
:class:`~pycose.keys.keyparam.OKPKpD` (for private COSE OKP keys) or :class:`~pycose.keys.keyparam.OKPKpX` (for public COSE
OKP keys) key attributes. These attributes are mandatory for a valid COSE OKP Key. If you don't specify them,
the :meth:`~pycose.keys.cosekey.CoseKey.from_dict` will throw an exception.
